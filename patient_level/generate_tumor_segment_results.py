...
if __name__ == '__main__':
  ...
  with open(config_json_file) as json_data:
    ...
    tumor_dataset_folder = os.path.join(quip_application_folder, "img/tumor_dataset");
    if not os.path.exists(tumor_dataset_folder):
      print "img/tumor_dataset folder is NOT available, then create it."
      os.makedirs(tumor_dataset_folder);    
  ...
  for user_case_id in  process_list:    
    ...
    new_execution_id=user +"_Tumor_Region_Dataset";   
    ...
    for index1 in range (0,prefix_index):
      ...
      out_folder = os.path.join(tumor_dataset_folder, case_id);
  ...
  for item in image_user_list:  
    ...
    user=item[1]; 
    execution_id=user +"_Tumor_Region";    
    execution_id2=user +"_Tumor_Region_Dataset";   
  ...
  for item in image_user_list:  
    case_id=item[0]; 
    print case_id ,tumor_dataset_folder;
    subprocess.call(shlex.split('./load_tumor_segment_results.sh' + ' ' + case_id + ' ' + tumor_dataset_folder ));
